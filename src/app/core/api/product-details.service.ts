...
import { ApiCategoryRef, ApiProductRecord } from "../../models/product/product-api.model";
import {
  ProductDetails,
...
} from "../../models/product/product-details.model";
@Injectable({
  providedIn: "root",
})
export class ProductDetailsService {
  private readonly http = inject(HttpClient);
  private readonly productCollectionsService = inject(
    ...
    if (product.detail || product.subtitle) {
      fallbackSections.push({
        ...
        lines: this.normalizeLines(
          [product.detail, product.subtitle].filter((line): line is string =>
            Boolean(line),
          ),
        ),
      });
  ...
  private inferSize(product: ProductDetailsApiRecord): string {
    return (
      ...
      this.cleanText(product.subtitle) ||
      "Standard size"
    );
  }
